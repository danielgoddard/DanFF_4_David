...
def get_data(options):
	...
	if options['data_type']=='custom':
		...
		mean_signal = np.sum(flux)/np.size(flux)

		wavelength  = initial_initial_data_wavelength
		ra          = 0#options['ra']
		dec         = 0#options['dec']
		header_test=pyfits.open('./DR7_sigma_catalogue.fits')
...
#	if options['data_type'] == 'test_manga' or options['data_type'] == 'manga':
#
#
...
#		
#		''' Replaces initial string to find RA, DEC from DRP file. '''
#		if options['radial_or_rss'] == 'rss':
#			name=options['file_in']
#			initial=name.replace('/mnt/lustre/eme/goddard/firefly_public_old/data/manga_rss/manga-','')
#			final=initial.replace('-LOGRSS_BIN-RADIAL-002.fits','')
#			i=final[0]
#			j=final[1]
#			k=final[2]
#			l=final[3]
#			file=pyfits.open('/mnt/lustre/westfall/MaNGA/redux/v1_3_3/'+i+j+k+l+'/stack/manga-'+str(final)+'-LOGRSS.fits.gz')
#			ex_data=file[0].header
#			ra=ex_data['OBJRA']
#			dec=ex_data['OBJDEC'] 
#			objid   = ex_data['MANGAID']
#			
#		''' Uses the standard FIREFLY procedure of RA, DEC from DAP file. '''
#		if options['radial_or_rss'] == 'radial':
...
#
#
#		pos_data= header[2].data
#		xpos    = pos_data['BINXRL'][bin_number]
#		ypos    = pos_data['BINYRU'][bin_number]
#
#
#		#redshift = 0.0
#		bad_flags = np.ones(len(flux))
